...
All other subsets of a set are *proper subsets*.
...
How can we prove that two sets $A$ and $B$ are equal? One option is to prove two subset claims:
...
This proof pattern is called **double inclusion** (or **mutual inclusion**); [Proposition 1.18 in Oxford's open *Introduction to University Mathematics* notes](https://courses.maths.ox.ac.uk/mod/page/view.php?id=65661) gives the same result and a worked set-identity proof.

Why is it valid? Suppose first that $A\subseteq B$ and $B\subseteq A$. Fix an arbitrary object $x$. If $x\in A$, the first inclusion gives $x\in B$. If $x\in B$, the second gives $x\in A$. Thus,

$$x\in A \quad\Longleftrightarrow\quad x\in B.$$

The two sets have exactly the same elements, so they are equal. The converse is immediate: if $A=B$, every element of either set is also an element of the other, so both inclusions hold.

We will reuse this argument for languages, since a language is a set of strings. To prove $L_1=L_2$, we can show that every string in $L_1$ is in $L_2$ and that every string in $L_2$ is in $L_1$. Later, when we compare grammar formalisms, the objects on the two sides will be *sets of languages*, but the reason double inclusion works will be exactly the same.
...
```{python}
emptyset: frozenset[object] = frozenset()
```
...
```{python}
vowels_with_empty: set[str | frozenset[object]] = set(vowels)
vowels_with_empty.add(emptyset)
...
The *union* of two sets is the set of elements in at least one of them. For instance, if we take the union of the set of high vowels with the set of back vowels, we get the set of high and/or back vowels.
