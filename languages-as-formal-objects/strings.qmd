...
Strings of length $N$ are given by $\Sigma^N$ and the [**Kleene closure**](https://en.wikipedia.org/wiki/Kleene_star) of $\Sigma$—notated $\Sigma^*$—gives us the set of all such sets.
...
$|\Sigma^0| = 1$ (just the empty string), $|\Sigma^1| = 3$, $|\Sigma^2| = 9$, and in general $|\Sigma^k| = 3^k$. Since $\Sigma^* = \bigcup_{k=0}^{\infty} \Sigma^k$ is a countably infinite union of finite sets of increasing size, $\Sigma^*$ is countably infinite.
:::
