...
The backreference `\1` preserves whatever precedes /ʃən/, and `\2` preserves /ʃən/. The negative lookbehind leaves *accreditation* alone because its suffix is /eɪʃən/. But the pattern has no access to morphological structure, so it overapplies to monomorphemic words such as *fashion* and *passion* that merely end in the same phone sequence.
...
The substitution `re.sub(r'(.+)t$', r'\1ʃən', stem)` is doing exactly what the morphophonological rule says: it captures everything before the final /t/, drops the /t/, and appends /ʃən/. And because we check whether the predicted noun form actually exists in the CMU dictionary, we're verifying that the rule produces an attested pronunciation. This check alone does not establish that the matched noun is morphologically related to the verb; homophones can create false positives.
...
def nominalize(m: re.Match[str]) -> str:
    ...
    retained_cluster = final_cluster[:-1]

    if len(final_cluster) > 1:
        return stem + retained_cluster + 'eɪʃən'
    else:
        return stem + 'ʃən'
...
As we'll see in the [phonological patterns module](../phonological-patterns/phonological-rules-as-fsas/finite-state-transducers.qmd), rewrite rules with regular targets and contexts can be compiled into *finite-state transducers* under the application modes developed there. The pattern in `re.sub` defines when the rule applies; the replacement defines what it does. The particular substitutions on this page copy a matched stem and add or remove a fixed string, so they can be represented by regular relations. Python's full regular-expression and replacement syntax also includes backreferences and other features that fall outside this finite-state result.
