...
In this section, we'll look at a simple but effective approach to morphological segmentation: [byte pair encoding](https://en.wikipedia.org/wiki/Byte_pair_encoding) (BPE), which was originally developed for data compression and later adapted for subword tokenization in NLP by @sennrich2016neural. If you've used a modern large language model, it almost certainly uses BPE (or a close relative) to tokenize its input, so this is a method worth understanding in its own right.
