...
To compute this efficiently, we use the Inside algorithm, which is a dynamic programming algorithm similar to the CKY algorithm. The Inside algorithm computes what are called "inside probabilities" $\alpha_{i,j}(A)$, which represent the probability that non-terminal $A$ derives the substring $w_i...w_{j-1}$.
