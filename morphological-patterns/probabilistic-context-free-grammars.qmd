...
So far, we've explored context-free grammars (CFGs) as powerful tools for modeling hierarchical structures in language. In this section, we'll extend our framework to [*probabilistic* context-free grammars](https://en.wikipedia.org/wiki/Probabilistic_context-free_grammar) (PCFGs), which augment CFGs with probabilities associated with each rule [@baker_trainable_1979]. This probabilistic extension allows us to:
...
To compute this efficiently, we use the [inside algorithm](https://en.wikipedia.org/wiki/Inside%E2%80%93outside_algorithm) [@baker_trainable_1979; @lari_estimation_1990], which is a dynamic programming algorithm similar to the CKY algorithm. The inside algorithm computes what are called "inside probabilities" $\alpha_{i,j}(A)$, which represent the probability that non-terminal $A$ derives the substring $w_i...w_{j-1}$.
...
\sum_{k=i+1}^{j-1} \sum_{B,C \in V} P(A \rightarrow B C) \cdot \alpha_{i,k}(B) \cdot \alpha_{k,j}(C) & \text{if } j > i + 1
\end{cases}
$$
