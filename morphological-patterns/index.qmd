...
This is one of the motivations for moving beyond finite state methods. In this module, we'll use two complementary families of models for morphology. For *segmentation*—finding where the morpheme boundaries are—we'll use sequence labeling models like [HMMs](../uncertainty-about-languages/beyond-ngrams.qmd) and [CRFs](segmentation-with-structured-prediction.qmd), which operate on flat character sequences (just as HMMs operate on phone sequences for phonotactics). For *parsing*—determining the hierarchical structure of how morphemes compose—we'll use [context-free grammars](operations.qmd) and their [probabilistic extensions](probabilistic-context-free-grammars.qmd). Segmentation feeds into parsing: once we know the morpheme boundaries, we can ask how those morphemes are structured.
