...
# The StrucAllo field contains allomorphic segmentations with
# morpheme boundaries marked.  We extract words that have a
# parseable segmentation.
import re

def extract_morphemes(strucallo):
    """Pull morpheme strings out of a CELEX StrucAllo field."""
    if pd.isna(strucallo):
        return None
    # Morphemes are enclosed in parentheses
    morphs = re.findall(r'\(([^)]+)\)', strucallo)
    if morphs:
        return [m.lower() for m in morphs]
    return None

celex['morphemes'] = celex.StrucAllo.apply(extract_morphemes)
