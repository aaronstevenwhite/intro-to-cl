...
One influential formalism for stating phonological rules comes from Chomsky and Halle's "The Sound Pattern of English" (SPE) [@chomsky1968sound]. In this formalism, rules have the general form:
...
The algorithm developed by @kaplan1994regular and refined by @mohri1997finite provides a systematic way to do this.
...
The formal algorithm for converting a rule $A \rightarrow B / C\_D$ into an FST involves several sophisticated steps. Here we follow the improved algorithm by @mohri1997finite, which fixed issues in the original @kaplan1994regular construction.
...
@mohri1997finite introduced three key improvements to the original Kaplan and Kay algorithm:
...
The CDRewrite function constructs $T$ using the improved marker transducer $T_{\text{Mark}}'$ (which includes the filters from above):
