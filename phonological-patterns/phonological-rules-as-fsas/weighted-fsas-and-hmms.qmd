...
- $\delta: Q \times \Sigma \rightarrow \mathcal{P}(Q)$ is the transition function (which may be partial), now returning sets of states to allow nondeterminism
- $\lambda: Q \times \Sigma \times Q \rightarrow \mathbb{R}$ assigns a weight to each transition (implicitly zero for non-transitions)
- $\rho: F \rightarrow \mathbb{R}$ assigns a weight to each final state

The weight of a path is the product (or sum, depending on the semiring) of the transition weights along the path, multiplied by the final-state weight. When the automaton is nondeterministic, the same string may be accepted by multiple paths; the weight of a string is then the sum (in the semiring) of the weights of all accepting paths for that string.
