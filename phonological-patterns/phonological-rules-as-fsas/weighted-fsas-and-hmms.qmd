...
The point is that an HMM *is* a nondeterministic WFSA. The nondeterminism corresponds to the hidden states: the same observation sequence can be produced by multiple state sequences (paths through the automaton), and the probability of the observation sequence is the sum of the probabilities of all such paths.

## Why this matters

This equivalence has practical consequences. All of the algorithms we've developed for finite state automata—composition, intersection, determinization, minimization—have weighted analogues that apply to WFSAs and therefore to HMMs. For instance:
...
We will make use of this connection in the [section on morphological segmentation](../../morphological-patterns/segmentation-with-structured-prediction.qmd), where HMMs are used to predict morpheme boundaries. The fact that HMMs are weighted automata means that the segmentation model is operating within the same formal framework as the phonological grammars and transducers we've been building throughout this module.

More generally, the equivalence between HMMs and weighted automata is an instance of a broader pattern: many of the probabilistic models used in computational linguistics—including $N$-gram models, HMMs, and probabilistic finite state transducers—can be understood as weighted versions of the formal machines we've already studied. This perspective is useful because it means that the algebraic properties of these machines (closure under operations, decidability of various questions) carry over to the probabilistic setting, giving us formal guarantees about what can and cannot be computed.
